...
### 1.5 Profile
- **Definition:** A **Profile** is a curated selection of rules from the baseline rule catalog, with optional mappings to an external framework or standard.
- **Role:** Profiles allow the same canonical Magebean rules to be reused for different assessment goals without duplicating rule definitions. For example, `--profile=owasp` selects application security rules mapped to OWASP categories, while `--profile=pci` selects payment and PCI DSS readiness rules.
...
- Align Magento security checks with **OWASP Top 10**, PCI DSS readiness needs, and industry standards.
...
- **MB-R086 — No raw card collection in Magento checkout (Critical, PCI Req. 3/6)**
  *Description:* Magento custom checkout code must not collect raw card numbers, CVV, magnetic stripe data, or equivalent sensitive authentication data unless the store is explicitly designed and assessed for that scope. Search templates, JavaScript, controllers, GraphQL handlers, and API payloads for direct card collection patterns. Prefer hosted payment pages, hosted fields, tokenization, and payment provider SDKs that reduce PCI scope.

- **MB-R094 — Authorization checks on custom controllers and APIs (High, A01)**
  *Description:* Custom controllers, REST routes, GraphQL resolvers, and admin actions must enforce authorization before exposing account, order, customer, or configuration data. Missing ACL checks create broken access control vulnerabilities that can leak sensitive data or permit unauthorized state changes. Validate use of Magento ACL resources, customer ownership checks, and explicit authorization guards.

- **MB-R095 — GraphQL and API exposure minimized (Medium, A01/A05)**
